...
async function waitReady(child) {
  return new Promise((resolve, reject) => {
    let text = '';
    const timer = setTimeout(() => reject(new Error(`bridge did not become ready: ${text}`)), 6000);
...
test('desktop bridge exposes real NativeSwarm state and dispatches through demo runtime', async t => {
  ...
  const focus = await call(base, '/api/focus', { method: 'POST', body: JSON.stringify({ agentId: 'engineer' }) });
  assert.equal(focus.state.swarm.matrix.activeFocus, 'engineer');
});
