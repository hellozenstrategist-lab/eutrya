...
test('browser action fetches and formats web page content', async t => {
  const server=http.createServer((_req,res)=>{res.writeHead(200,{'content-type':'text/html'});res.end('<html><head><title>Example Domain</title></head><body><h1>Example Domain</h1><a href="/more">More</a></body></html>');});
  await new Promise((resolve,reject)=>server.once('error',reject).listen(0,'127.0.0.1',resolve));
  t.after(()=>server.close());
  const url=`http://127.0.0.1:${server.address().port}/`;
  const { runtime } = rig(t, {
    actions: [{ type: 'browser', url }]
  });
  await runtime.run();
  const r = runtime.state.observations[0].result;
