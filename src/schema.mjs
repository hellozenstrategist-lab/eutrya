...
export const ACTION_HELP = {
  ...
  run: 'Request an operator-approved process by executable + args: {type:"run",program:"git",args:["clone","https://github.com/org/repo.git","repo"]}. Use for approved local tooling (npm, git, tests, analyzers); the program may use the network. Only available when the operator enables execution.',
  shell: 'Execute a command in the workspace shell (requires operator approval): {type:"shell",command:"ls -la"}',
  browser: 'Read a live HTTP(S) page with headless Chromium or fetch and return readable content plus links: {type:"browser",url:"https://example.com"}. Use it for public bounty/program pages, documentation, specifications, and linked source pages; it does not require local process execution.',
